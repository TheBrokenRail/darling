...
{
	if (verbose) puts("STUB: AudioUnitALSA::reset");
	return unimpErr;
}
...
{
	if (verbose) puts("STUB: AudioUnitALSA::initInput");
...
void AudioUnitALSA::processAudioEvent(struct pollfd origPoll, int event)
{
	...
	(void) snd_pcm_poll_descriptors_revents(m_pcmOutput, &pfd, 1, &revents);
...
void AudioUnitALSA::startInput()
{
	...
	if (verbose) puts("STUB: AudioUnitALSA::startInput");
