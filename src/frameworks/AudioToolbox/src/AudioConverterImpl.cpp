#include <AudioToolbox/AudioConverterImpl.h>
...
OSStatus AudioConverter::allocateBuffers()
{
...
OSStatus AudioConverter::setProperty(AudioConverterPropertyID inPropertyID, UInt32 inPropertyDataSize, const void *inPropertyData)
{
	switch (inPropertyID)
	{
		...
		default:
		{
			if (verbose) puts("STUB: AudioConverter::setProperty");
			return kAudioConverterErr_PropertyNotSupported;
		}
	}
...
{
	if (verbose) puts("STUB: AudioConverter::getPropertyInfo");
	return unimpErr;
}
...
OSStatus AudioConverter::getProperty(AudioConverterPropertyID inPropertyID, UInt32 *ioPropertyDataSize, void *outPropertyData)
{
	switch (inPropertyID)
	{
		...
		default:
		{
			if (verbose) puts("STUB: AudioConverter::getProperty");
			return kAudioConverterErr_PropertyNotSupported;
		}
	}
}
